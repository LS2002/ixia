...
def waitForActionToFinish(connection, replyObj, actionUrl):
    ...
    if action_result_url:
        ...
        if not success:
            error_msg = 'Error while executing {}. '.format(actionUrl)
...
def performGenericGet(connection, listUrl, errors=None):
    """
    ...
    if errors is None:
        errors = []
    reply = connection.httpGet(url=listUrl, errorCodes=errors)
...
def downloadResource(connection, downloadFolder, localPath, zipName="", timeout=80):
    """
        ...
        - zipName is the name that archive will take, this parameter is mandatory only for folders,
                  if you want to download a file this parameter is not used.
    """
    ...
    downloadResourceReply = connection.httpRequest('GET', downloadResourceUrl, params=parameters, downloadStream=True,
                                                   timeout=timeout)

    if not downloadResourceReply.ok:
        raise Exception(
            "Error on executing GET request on url %s: %s" % (downloadResourceUrl, downloadResourceReply.text))
...
def startNewSession(connection):
    ...
    log("IxLoad: created session no %s" % sessionId)
...
def uploadFile(connection, url, fileName, uploadPath, overwrite=True):
    ...
    try:
        ...
        raise Exception(
            'Upload file failed. Received connection error. One common cause for this error is the size of the file '
            'to be uploaded. '
...
def releaseConfiguration(connection, sessionUrl):
    """
    This method is used to release the currently loaded test. After starting the 'Release Config' action,
...
def runTest(connection, sessionUrl):
    """
        This method is used to start the currently loaded test. After starting the 'Start Test' action,
        wait for the action to complete.
...
def enableAnalyzerOnPorts(connection, sessionUrl, communityPortIdTuple):
    ...
    patchUrl = sessionUrl + "/ixload/test/activeTest/communityList/%s/network/portList/%s" % (
        communityObjectID, portObjectID)
...
def getTestCurrentState(connection, sessionUrl, timeout=80):
    """
    ...
    """
    activeTestUrl = "%s/ixload/test/activeTest" % sessionUrl
    testObj = connection.httpGet(activeTestUrl, timeout)
...
def waitForTestToReachUnconfiguredState(connection, sessionUrl, timeout=80):
    """
    ...
    Args:
        ...
        - sessionUrl is the address of the session that should run the test.
    """
    while getTestCurrentState(connection, sessionUrl, timeout) != kTestStateUnconfigured:
        time.sleep(0.1)
...
def assignPorts(connection, sessionUrl, portListPerCommunity):
    ...
    for communityName in portListPerCommunity:
        if communityName not in communityNameList:
            errorMsg = "Error while executing assignPorts operation. " \
                       "Invalid NetTraffic name: %s. This NetTraffic is not defined in the loaded rxf." % communityName
...
def setCardsAggregationMode(connection, chassisChainUrl, chassisIp, cardIdList, mode):
    """
        ...
        - chassisIp is the IP or hostname of the chassis that contains the card(s)
        - cardIdList is a list of card IDs
        - mode is the aggregation mode that will be set on the cards. Possible options are (depending on card type):
         NA (Non Aggregated), 1G, 10G, 40G
    """
...
def getIPRangeListUrlForNetworkObj(connection, networkUrl):
    ...
    if isinstance(networkObj, list):
        ...
        for link in networkObj.links:
            if link.href.split("/")[-1] == 'rangeList':
                rangeListUrl = normalizeLink(link.href)
                return rangeListUrl

        for link in networkObj.links:
            if link.href.split("/")[-1] == 'childrenList':
...
def addCommands(connection, sessionUrl, commandDict):
    """
        ...
        - connection is the connection object that manages the HTTP data transfers between the client and the REST API
        - sessionUrl is the address of the session that should run the test
        - commandDict is the Python dict that holds the mapping between agent name and specific commands.
          (commandDict format -> { agent name : [ { field : value } ] })
    """
        ...
        for action in actionOrder:
            if action in configDict:
                for networkInfo in configDict[action]:
                    ...
                    dutRangesInfo[networkInfo] = actionDict[action](connection, dutListUrl,
                                                                    configDict[action][networkInfo])
...
def deleteAllSessions(connection):
    s = connection.httpDelete('sessions')
...
class NetworkUtils(object):
        ...
        for node in nodes:
            if node.name == pluginName:
                for link in node.links:
                    ...
                    raise Exception(
                        'Plugin %s does not have a rangeList with rangeListType %s' % (pluginName, rangeListType))
            else:
    ...
    def getRangeListUrl(connection, sessionUrl, communityName, pluginName, rangeListType):
        ...
        if rangeListUrl is None:
            raise Exception('Plugin %s under community %s does not have a rangeList of type %s.' % (
                pluginName, communityName, rangeListType))
        return rangeListUrl
    ...
    def getRangeUrl(connection, sessionUrl, communityName, pluginName, rangeListType, rangeName):
        ...
        if r is None:
            raise Exception(
                'Community %s, plugin %s does not have a range named %s.' % (communityName, pluginName, rangeName))
    ...
    def changeRangeOptions(connection, sessionUrl, communityName, pluginName, rangeListType, rangeName, rangeOptions):
        rangeUrl = NetworkUtils.getRangeUrl(connection, sessionUrl, communityName, pluginName, rangeListType, rangeName)
        if rangeUrl is None:
            raise Exception('Invalid range name %s for community %s, plugin %s and rangeListType %s' % (
                rangeName, communityName, pluginName, rangeListType))
    ...
    def addPlugin(connection, sessionUrl, communityName, parentPluginName, pluginOptions):
        ...
        if parentPluginChildrenUrl is None:
            raise Exception('Invalid parent plugin name %s under community %s' % (parentPluginName, communityName))
...
class POP3Utils(ActivityNetworkMixinUtils):
    ...
    def changeMailMessageType(connection, sessionUrl, communityName, activityName, commandNumber, optionsDict):
        ...
        for POP3Command in POP3Commands:
            ...
            raise Exception('Community %s, activity %s does not have a command with the number %s' % (
                communityName, activityName, commandNumber))
...
class StatelessPeerUtils(ActivityNetworkMixinUtils):
    ...
    def addStatelessPeerCommand(connection, sessionUrl, communityName, activityName, optionsDict):
        activityListUrl, activity = StatelessPeerUtils.getActivityByName(connection, sessionUrl, communityName,
                                                                         activityName)
...
class SMTPUtils(ActivityNetworkMixinUtils):
    ...
    def changeMailMesage(connection, sessionUrl, communityName, activityName, commandName, optionsDict):
        ...
        for smtpCommand in smtpCommands:
            ...
            raise Exception('Community %s, activity %s does not have a command named %s' % (
                communityName, activityName, commandName))
